...
$query = "SELECT username, characteristics FROM users WHERE loginreqkey='$req'";

if ($stmt = $mysqli->prepare($query)) {
    $stmt->execute();
    $stmt->bind_result($gusername, $characteristics);
    while ($stmt->fetch()) {
     echo "{";
        echo '"username": "' . $gusername . '",';
...
if($_GET["func"]=="u_characteristics"){
...
$sql = "UPDATE users SET characteristics='$characteristics' WHERE loginreqkey='$req'";
